...
import { useCompensationList, useCompensationSummary } from "../hooks/useCompensation";
...
function SummarySkeleton() {
  return (
    <div className="grid grid-cols-2 gap-x-6 gap-y-5 sm:grid-cols-4" aria-hidden="true">
...
export default function ExplorerPage() {
  ...
  const patch = (p: Partial<FilterValues> & { sort?: string; order?: string }) => {
  ...
  const query: CompensationQuery = {
    ...
    minTC: minTC && Number.isFinite(Number(minTC)) ? Number(minTC) * 100000 : undefined,
    maxTC: maxTC && Number.isFinite(Number(maxTC)) ? Number(maxTC) * 100000 : undefined,
    sort,
    order,
    page,
    limit: LIMIT,
  ...
  useEffect(() => {
    if (!list.loading && list.total > 0 && list.records.length === 0 && page > 1) {
      goToPage(1);
    }
  ...
  const handleSort = (field: string) => {
    patch({ sort: field, order: sort === field && order === "desc" ? "asc" : "desc" });
  };

  const values: FilterValues = { roleId, levelId, locationId, companyId, minTC, maxTC };
  ...
  const tableCols = 6 + (showRoleColumn() ? 1 : 0) + (showLocationColumn() ? 1 : 0);
  ...
  return (
    <PageContainer>
      <header className="border-b border-line pb-6">
        <h1 className="text-2xl font-semibold tracking-tight">Compensation Explorer</h1>
        <p className="mt-1.5 text-[15px] text-muted">See what comparable roles are actually paid.</p>
      </header>
      ...
      {hasContext && (
        <div className="mt-9 flex flex-wrap items-center gap-x-2.5 gap-y-2 text-[17px]">
          <span className="font-semibold tracking-tight">{ctxRole ?? "All roles"}</span>
        ...
        <Section
          ...
          aside={
            !summary.loading && !summary.error && summary.data
              ? `Based on ${formatCount(summary.data.sampleCount)} records`
              : undefined
          }
        >
          {summary.error ? (
            ...
            <>
              ...
              <div className="mt-8">
                <h3 className="text-[13px] font-medium text-ink">Median package</h3>
                <div className="mt-3">
      ...
      <Section
        ...
        aside={!list.loading && !list.error ? formatRange(page, LIMIT, list.total) : undefined}
      >
        {list.error ? (
          ...
          <div className="overflow-hidden rounded border border-line bg-surface">
            ...
            />
            <Pagination page={page} limit={LIMIT} total={list.total} onPage={goToPage} />
          </div>
        )}
      </Section>
    </PageContainer>
